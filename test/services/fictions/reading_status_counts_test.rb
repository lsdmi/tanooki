...
module Fictions
  class ReadingStatusCountsTest < ActiveSupport::TestCase
    setup do
      @fiction = fictions(:one)
      @chapter = chapters(:one)
    end

    test 'returns reading counts in status order' do
      ...
      result = ReadingStatusCounts.new(fiction: @fiction).call
    ...
    test 'uses one grouped count query' do
      ...
      ReadingStatusCounts.new(fiction: @fiction).call
