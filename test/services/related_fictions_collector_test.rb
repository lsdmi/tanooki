...
class RelatedFictionsCollectorTest < ActiveSupport::TestCase
  setup do
  ...
  test 'collect returns related fictions from reading progress' do
    results = RelatedFictionsCollector.new([@reading]).collect
  ...
  test 'collect returns empty array when readings are empty' do
    assert_empty RelatedFictionsCollector.new([]).collect
  end

  test 'collect respects exclude_ids' do
    results = RelatedFictionsCollector.new(
      [@reading],
  ...
  test 'collect does not duplicate the same related fiction' do
    results = RelatedFictionsCollector.new([@reading, @reading]).collect
