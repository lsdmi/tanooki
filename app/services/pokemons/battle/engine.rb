  ...
  module Battle
    ...
    class Engine
      ...
      def initialize(attacker_team, defender_team, logger)
        @attacker_side_team = SideTeam.new(attacker_team)
        @defender_side_team = SideTeam.new(defender_team)
        @logger = logger
      end
      ...
      def apply_character_effects(attacker, defender, attacker_stats, defender_stats)
        CharacterEffects.apply(attacker, defender, attacker_stats, defender_stats)
      end
      ...
      def calculate_type_effectiveness(stats, own_pokemon, opponent_pokemon)
        TypeEffectiveness.calculate(stats, own_pokemon, opponent_pokemon)
      end
      ...
      def handle_battle_result(participants)
        attacker_result = calculate_battle_result(participants[:attacker_stats])
        defender_result = calculate_battle_result(participants[:defender_stats])
