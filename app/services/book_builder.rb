...
class BookBuilder
  ...
  def add_cover(book)
    cover_title = @volume_title || Books::EpubChapterHtml.title(@chapters.first)
  ...
  def add_single_chapter(book, chapter)
    book.add_item('text/content.xhtml')
        .add_content(StringIO.new(Books::EpubChapterHtml.html(chapter)))
        .toc_text(Books::EpubChapterHtml.title(chapter))
        .landmark(type: 'bodymatter', title: Books::EpubChapterHtml.title(chapter))
  end
